...
if [ $# == '0' ]; then
    ...
    echo "    bash $0 2 linear Llama-2-7B 1"
    echo "    bash $0 4 memory Llama-2-7B 1 64 0.9"
    exit
fi
...
if [ "$model_type" == "linear" ]; then
    ...
    ACCELERATE_LOG_LEVEL=info uv run accelerate launch --config_file llm/configs/ddp.yaml --num_processes ${nprocs} \
        -m llm.train_linear --config llm/configs/config_linear-llama.yaml --model_name_or_path ${model_path} \
    ...
    ACCELERATE_LOG_LEVEL=info uv run accelerate launch --config_file llm/configs/ddp.yaml --num_processes ${nprocs} \
        -m llm.train_memory --config llm/configs/config_memory-llama.yaml --model_name_or_path ${model_path} \
